...
def main():
    ...
    # Configuration
    input_file = '_0_task1/buiding.json'  # Replace with your CityJSON file path
    ...
    try:
       ...
       # Save to CSV
        ...
        # 1) Gather all unique base IDs (prefixes)
        prefixes = set(item["object_id"].split("-", 1)[0] for item in results)
        print(prefixes)
        ...
        for prefix in prefixes:
                for item in results:
                    ...
                    areas_lod_1_2_ground = [
                        item["area_2d"]
                        for item in results
                        if (
                            item["object_id"].startswith(prefix)
                            and item["object_type"] == "BuildingPart"
                            and item["geometry_type"] == "Solid"
                            and item["lod"] == "1.2"
                            and item["semantic_type"] == "GroundSurface"
                        )
                    ]
                areas_by_entity[prefix]=areas_lod_1_2_ground[0]
        print(areas_by_entity)
        input("..............;")


        input(areas_lod_1_2_ground)
        # Print summary
